...
async function getFinishedOpportunities() {
  try {
    const { getOpportunities } = await import('@/lib/services/opportunity-service');
    const all = await getOpportunities();
    return all.filter(opp => opp.status === 'WON' || opp.status === 'LOST');
  } catch (error) {
...
export default async function OportunidadesFinalizadasPage() {
  const opportunities = await getFinishedOpportunities();
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Oportunidades Finalizadas</h1>
