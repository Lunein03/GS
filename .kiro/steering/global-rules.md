...
### Docker Environment
- **ALWAYS check Docker container status** before suggesting database operations or connections
- Use `docker ps` to verify running containers (database, services, etc.)
- The project uses Docker for database and related services
- Before suggesting database migrations, connections, or queries:
  - Verify Docker containers are running
  - Check if the database container is healthy
  - Use `docker-compose` commands when appropriate
...
- If containers are not running, suggest starting them before proceeding with database operations
...
#### MCP Negative Prompts (What NOT to Do)

**NEVER do these actions - ALWAYS use MCP tools instead:**

- ❌ **DO NOT** suggest manual file operations without using MCP file tools
- ❌ **DO NOT** skip the sequential thinking MCP before complex tasks
- ❌ **DO NOT** execute commands directly without planning via MCP thinking
- ❌ **DO NOT** make assumptions without analyzing context via MCP
- ❌ **DO NOT** proceed with implementation before using `mcp_sequential_thinking_sequentialthinking`
- ❌ **DO NOT** suggest "you can do X" without actually doing it via MCP
- ❌ **DO NOT** provide instructions for the user to execute manually when MCP tools are available
- ❌ **DO NOT** skip validation steps after executing MCP actions
- ❌ **DO NOT** use shell commands for file operations when MCP file tools exist (readFile, fsWrite, strReplace, etc.)
- ❌ **DO NOT** bypass the mandatory workflow (Analysis → Planning → Execution → Validation)

**If you find yourself about to:**
- Suggest a manual action → STOP and use the appropriate MCP tool
- Skip thinking → STOP and use `mcp_sequential_thinking_sequentialthinking`
- Execute without planning → STOP and create a plan via MCP thinking first
- Provide instructions instead of doing → STOP and execute via MCP tools directly
...
**Always respond in Portuguese to the user** - Sempre responda em português ao usuário.
